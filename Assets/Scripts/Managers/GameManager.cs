...
public class GameManager : Singleton<GameManager>
{
    ...
    public List<Movable> gors, skrals, trolls, wardraks;
    ...
    void Start()
    {
        ...
        skrals = new List<Movable>();
        //skrals.Add(Skral.Factory(19));

        trolls = new List<Movable>();
        wardraks = new List<Movable>();
     ...
     */
    void monsterMove(List<Movable> enemy)
    {
        if (enemy != null)
        {
            foreach (var monster in enemy)
            {
                ...
                do
                {
                    if (nextCell.State.Enemy == null)
                    {

                        // update monster's prev cell's enemy state
                        monster.Cell.State.Enemy = null;

                        // move monster to this cell
                        monster.Move(nextCell);

                        nextCell.State.Enemy = (IEnemy)monster;
